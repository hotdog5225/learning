...
class DepartmentInfo:
    ...
    def dump_department_info(self):
        url = 'https://www.114yygh.com/web/department/hos/list?_time=1637346232588&hosCode=12'
        ...
        BeiJingDaXueDiYiYiYuan_hosp_id = 12
        with open('./BeiJingDaXueDiYiYiYuan_dept_info.json', 'r') as f:
            department_info_dict = json.loads(f.read())
            self.write_department_info_to_db(department_info_dict['data']['list'], BeiJingDaXueDiYiYiYuan_hosp_id)
        print("DONE: department info dumped")

    def get_all_dept_info_by_hosp_id(self, hosp_id):
        cusor = self.connection.cursor()
        sql = 'select * from department where hosp_id = {}'.format(hosp_id)
        cusor.execute(sql)
        dept_list = cusor.fetchall()
        return dept_list

if __name__ == '__main__':
    dept = DepartmentInfo()
    dept.dump_department_info()